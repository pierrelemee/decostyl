...
for host in $(echo "${HOSTS}" | tr ";" "\n"); do
    ...
    ssh "${host}" 'test -d /var/www/decostyl || mkdir -p /var/www/decostyl'
done
...
if [ -d "" ]; then
    ...
    # Upload code
    for host in $(echo "${HOSTS}" | tr ";" "\n"); do
      scp -rqC "${TEMP_DIR}" "${host}:/var/www/decostyl/${VERSION}"
    done
    # Shared files
    for host in $(echo "${HOSTS}" | tr ";" "\n"); do
      shared_files=$(ssh "${host}" 'find /var/shared/decostyl -printf "%P\n"')
      for shared_file in ${shared_files}; do
          echo "Symlinking shared file ${shared_file}"
          ssh "${host}" "ln -snf /var/shared/decostyl/${shared_file} /var/www/decostyl/${VERSION}/${shared_file}"
      done
    done
    # Install PHP dependencies w/ composer
    for host in $(echo "${HOSTS}" | tr ";" "\n"); do
      echo "Install PHP dependencies w/ composer"
      ssh "${host}" "cd /var/www/decostyl/${VERSION} && composer --no-progress --no-cache --no-interaction --no-dev install"
    done
    # Update symlinks
    for host in $(echo "${HOSTS}" | tr ";" "\n"); do
      echo "Update symlinks"
      ssh "${host}" "ln -snf /var/www/decostyl/${VERSION} /var/www/decostyl/current"
    done
    # Reload nginx
    for host in $(echo "${HOSTS}" | tr ";" "\n"); do
      echo "Reload nginx"
    ...
    # Remove older builds
    for host in $(echo "${HOSTS}" | tr ";" "\n"); do
      ssh "${host}" 'for build in $(find /var/www/decostyl -maxdepth 1 -type d  | grep -v "^$" | sort | head -n -5); do rm -Rf "/var/www/decostyl/${build}"; done'
    done
fi
